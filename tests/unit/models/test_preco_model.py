...
class TestPrecoModel:
    def test_create_preco(self):
        """Testa a criação de uma instância de Preco com os campos obrigatórios."""
        preco = Preco(seller_id="1", sku="A", preco_de=100, preco_por=90)
    ...
    def test_update_preco(self):
        """Testa a atualização dos campos de uma instância de Preco."""
        preco = Preco(seller_id="1", sku="A", preco_de=100, preco_por=90)
    ...
    def test_from_json(self):
        """Testa a criação de uma instância de Preco a partir de um JSON."""
        json_data = '{"seller_id": "1", "sku": "A", "preco_de": 100, "preco_por": 90}'
        preco = Preco.from_json(json_data)
    ...
    def test_create_preco_with_missing_fields_raises_error(self):
        """Testa se a criação de Preco sem campos obrigatórios levanta erro."""
        # seller_id ausente
        with pytest.raises(ValidationError):
            Preco(sku="A", preco_de=100, preco_por=90)
        # sku ausente
        with pytest.raises(ValidationError):
            Preco(seller_id="1", preco_de=100, preco_por=90)
        # preco_de ausente
        with pytest.raises(ValidationError):
            Preco(seller_id="1", sku="A", preco_por=90)
        # preco_por ausente
        with pytest.raises(ValidationError):
            Preco(seller_id="1", sku="A", preco_de=100)
    ...
    def test_from_json_with_invalid_json_raises_error(self):
        ...
        with pytest.raises(Exception):
            Preco.from_json(json_data)

    def test_preco_accepts_zero_and_negative_values(self):
        """Testa se Preco aceita valores zero e negativos para os preços."""
        preco = Preco(seller_id="1", sku="A", preco_de=0, preco_por=-10)
        assert preco.preco_de == 0
        assert preco.preco_por == -10

    def test_preco_str_representation(self):
        """Testa se a representação em string de Preco contém informações relevantes."""
        preco = Preco(seller_id="1", sku="A", preco_de=100, preco_por=90)
        s = str(preco)
